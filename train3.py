...
for epoch in range(iter_times):
    ...
    with tqdm(enumerate(data_loader), total=dataset_size, ncols=40) as pbar: # len(data_loader)
        for batch, (content_imgs, _) in pbar:
            ...
            # style loss
            #style_loss = F.mse_loss(output_features[1], style_features[1]) # TODO [1]
            ...
            # content loss
            content_loss = 0
            content_loss += F.mse_loss(output_features[1], content_features[1])
            ...
            for og, sg in zip(output_gram, content_gram):
                content_loss -= F.mse_loss(og, sg.expand_as(og)) # expand the sg to the same size of og which doesn't allocate any memory, since zip will remain the shorter iterator
            style_loss = 0
